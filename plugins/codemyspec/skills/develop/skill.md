...
!`curl -sS --max-time 30 -X POST "http://localhost:4003/api/skills/start" -H "X-Working-Dir: ${CLAUDE_PROJECT_DIR}" --data-urlencode "skill=develop" --data-urlencode "external_id=${CLAUDE_SESSION_ID}" --data-urlencode "arguments=$ARGUMENTS"`
