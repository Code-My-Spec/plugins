#!/usr/bin/env bash
#
# Logs Codex skill invocations to ~/.codex/skill_invocations.jsonl.
# Ships with the market-my-spec plugin and is wired up by hooks.json at
# the plugin root — there is nothing for the user to install by hand. Codex
# gates plugin hooks behind hook trust, so this does not run until approved.
#
# Registered as both a PreToolUse and UserPromptSubmit hook. It reads the hook
# event JSON from stdin and writes a JSONL line when it sees either:
...
LOG_FILE="${HOME}/.codex/skill_invocations.jsonl"
