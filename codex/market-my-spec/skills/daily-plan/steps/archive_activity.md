...
**Scope: we only archive skills we could have scaffolded.** That means project-local skills under `.codex/skills/<name>/`. User-global skills and plugin skills are infrastructure — we surface their non-use in the review report but don't touch them. If the user wants to remove a user-global or plugin skill, they do it manually or via `codex plugin remove`.

## Phase 1 — Locate the target

Given `<name>`, check project-local first:

1. `.codex/skills/<name>/` — project-local skill. Our target.
2. If not there, check `~/.codex/skills/<name>/` (user-global). If found, **do not archive** — tell the user:

   > "`/<name>` is a user-global skill at `~/.codex/skills/<name>/`, not project-local. `/daily-plan archive` only handles project-local skills — user-global skills are infrastructure you installed intentionally. To remove it, do it manually (`mv ~/.codex/skills/<name> ~/.codex/skills-archive/<name>`) or ignore if it's fine sitting idle. Want me to remove this activity from the project's roster instead?"

3. If not there either, check `~/.codex/plugins/cache/*/*/*/skills/<name>/` (plugin). If found:

   > "`/<name>` is a plugin skill. To remove it, uninstall the plugin with `codex plugin remove <plugin-name>@<marketplace>` (the `@<marketplace>` suffix is required — `codex plugin list` shows both). Want me to remove this activity from the project's roster, leaving the plugin alone?"
...
- Where it's going (`<project>/.codex/skills-archive/<name>/`)

Prompt:

> "Archive `/<name>`?
> - Source: `<project>/.codex/skills/<name>/`
> - Destination: `<project>/.codex/skills-archive/<name>/`
...
1. `mkdir -p .codex/skills-archive` (relative to project root; create once, idempotent)
2. `mv .codex/skills/<name> .codex/skills-archive/<name>`
   - If a `<name>` already exists in `skills-archive/`, append a date suffix: `.codex/skills-archive/<name>-YYYY-MM-DD/`
3. Write an archive marker inside the moved directory: `.codex/skills-archive/<name>/ARCHIVED.md`
...
> "`/<name>` archived. Moved to `.codex/skills-archive/<name>/` (commit this alongside `marketing/` if the repo is tracked). Roster marked archived. Restore with `/daily-plan add <name>`."
...
`/daily-plan add <name>` checks for `.codex/skills-archive/<name>/` and offers to restore instead of scaffold. See `steps/scaffold_activity.md` Phase 1.
