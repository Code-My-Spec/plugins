...
- If `~/.codex/skill_invocations.jsonl` exists: read the last 30 days. Group by skill, compute: count, first-used-in-window, last-used, frequency.
- If not (hooks not trusted yet, or nothing logged since install): fall back to grepping the session transcripts under `~/.codex/sessions/`:
  ```
  grep -rhoE 'skills/[a-z0-9-]+/SKILL\.md' ~/.codex/sessions/*/*/*/*.jsonl | sort | uniq -c | sort -rn
  ```
  Plus for slash commands — Codex transcripts store user prompts as plain text, so pull the ones that start with a slash:
  ```
  jq -r 'select(.type=="response_item") | .payload | select(.role=="user") | .content[0].text // empty' \
    ~/.codex/sessions/*/*/*/*.jsonl 2>/dev/null \
    | grep -oE '^/[A-Za-z][A-Za-z0-9:_-]*' | sort | uniq -c | sort -rn
  ```
  Session files are laid out by date (`~/.codex/sessions/YYYY/MM/DD/rollout-*.jsonl`), so the path itself gives you approximate last-used dates. Less precise than the hook but workable.
...
- **Installed skills sitting unused.** From the broader skill-usage data, surface any user-global skill (`~/.codex/skills/<x>`) or plugin skill that has zero uses in 30+ days. **Do not propose to archive these** — just report them so the user can decide to remove the plugin or manually clean up. They're infrastructure, not our scope.
...
- SEO audit → archived (move skill out of `.codex/skills/`)
- Add Elixir Forum post as active-gap

## Skill usage (broader, for awareness — we don't touch these)
- /seo-page: 8 uses
- /seo-technical: 0 uses in 30+ days  ← user-global, you might want to remove the plugin
- /scan-reddit: 14 uses
