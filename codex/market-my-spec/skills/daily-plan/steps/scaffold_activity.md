...
Creates a real Codex skill at `.codex/skills/<name>/SKILL.md` **in the current project** (not user-global) and adds a row to `marketing/activities.md`. The skill is minimum-viable — just enough to be useful on day 1. The user iterates on it later.

## Why project-local, not user-global

Scaffolded activities are project-specific — they're built against this project's strategy, personas, and voice. They don't belong in `~/.codex/skills/`, which would pollute every other project with this one's marketing skills.
...
- Archive cleanly (local `.codex/skills-archive/`, also committed)

Reserve `~/.codex/skills/` for genuine infrastructure the user installs globally (SEO tools, generic utilities) — typically via plugins.
...
1. **Project-local** — `ls .codex/skills/<name>` — if it exists, ask: "`.codex/skills/<name>` already exists in this project. Add to roster without scaffolding, replace it, or pick a different name?"
2. **Project-local archive** — `ls .codex/skills-archive/<name>` — if archived here, ask: "`<name>` is in this project's archive. Restore, or scaffold fresh?"
   - **Restore:** move `.codex/skills-archive/<name>/` back to `.codex/skills/<name>/`, update the existing `activities.md` row to `active`, done.
3. **User-global** — `ls ~/.codex/skills/<name>` — if it exists there, ask: "A user-global skill `/<name>` already exists at `~/.codex/skills/<name>/`. Add that to your roster (it'll work from this project too), or scaffold a project-local one that overrides it?"
4. **Installed plugin skill** — search `~/.codex/plugins/cache/*/*/*/skills/<name>/`. If found: "Plugin skill `/<name>` exists (from plugin `X`). Add that to your roster instead of scaffolding a duplicate?"
...
---
name: <name>
description: <one-sentence what-it-does, in the user's voice, with a "use when..." clause so Codex knows when to suggest it>. Part of the marketing daily rotation (channel: <channel>, loop: <loop>).
user-invokable: true
argument-hint: "[topic]"
---
...
**Frontmatter rules (Codex — get these exactly right):**

- `user-invokable: true` — spelled with a **k**. This is what gives the skill its slash command.
- `argument-hint` must be a **quoted string**. Drop the line entirely if the activity takes no argument; don't leave a placeholder.
- `name` and `description` are required. The description is how Codex decides when to load the skill — keep it rich and specific.
...
1. `mkdir -p .codex/skills/<name>` (relative to the project root — the working directory)
2. Write SKILL.md to `<project>/.codex/skills/<name>/SKILL.md` (Write tool needs an absolute path — resolve it from the working directory)

The Write tool requires absolute paths. Resolve the project root from the working directory (usually Codex's `cwd`), then build the absolute path as `<cwd>/.codex/skills/<name>/SKILL.md`.
...
> "Scaffolded `/<name>` at `.codex/skills/<name>/SKILL.md` (project-local — commit it alongside `marketing/` if you want it versioned). Added to roster as active. Restart Codex for the slash command to register. Edit the SKILL.md to refine — it's minimum-viable by design."
