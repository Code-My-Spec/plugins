...
- `codex mcp list --json` — registered MCP servers, with command + args + declared env
...
2. Apply its `detection:` block:
   - `type: mcp` — scan every registered server's command + args from `codex mcp list --json` (each entry's `transport.command` and `transport.args`). Match if any server's args contain any string in `args_contains`. For remote servers, match against `transport.url`. Detection is by command/args/url pattern, never by server key name (users name MCPs whatever they want).
   - `type: env` — check if `env_var` is present in project `.env`.
   - `type: file` — check if the `file_path` exists.
...
**A registered server with no `env_vars` line (and no wrapper) is `partial`, not `ready`** — it has no way to receive its credentials. Same for a server whose `env_vars` are declared but absent from `.env`. This is the most common half-installed shape.
...
| google-apis | absent | required | `/marketing-stack install google-apis` |
...
- **Asking the user what's installed.** Read the config yourself. Only ask when detection is genuinely ambiguous.
- **Calling a server `ready` just because it's registered.** Registration is cheap; credentials reaching the process is the actual bar.
