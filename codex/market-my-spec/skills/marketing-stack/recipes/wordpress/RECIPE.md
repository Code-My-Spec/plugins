...
2. **On the user's machine:**
   ```bash
   codex mcp add wordpress -- npx -y @automattic/mcp-wordpress-remote
   ```
3. Add `WP_API_URL` to `.env` pointing at the site, then name the variables in `~/.codex/config.toml` (names only — Codex does not expand `${VAR}`):
   ```toml
   [mcp_servers.wordpress]
   command = "npx"
   args = ["-y", "@automattic/mcp-wordpress-remote"]
   env_vars = ["WP_API_URL", "WP_API_USERNAME", "WP_API_PASSWORD"]
   ```
   (Include the username/password names only if you're on the Application Passwords path rather than OAuth.)
4. Load `.env` and start Codex from that shell (`set -a; . ./.env; set +a` then `codex`).
5. First call triggers the OAuth 2.1 browser flow. User authorizes. Token cached locally by mcp-wordpress-remote.

Undo: `codex mcp remove wordpress` + delete the `.env` block. The server-side plugin is deactivated separately in WP Admin.
...
## Validation
Ask Codex: "Using the WordPress MCP, list my 5 most recent posts." Expect post IDs + titles.
