...
7. **Register the MCP:**
   ```bash
   codex mcp add postiz -- npx -y @gitroom/postiz-mcp
   ```
   (Package name as of 2026-04; verify in postiz-app/libraries/mcp/README.md.)
8. Add env vars to `.env`, then name them in `~/.codex/config.toml` (names only — the API key never lands in this file):
   ```toml
   [mcp_servers.postiz]
   command = "npx"
   args = ["-y", "@gitroom/postiz-mcp"]
   env_vars = ["POSTIZ_URL", "POSTIZ_API_KEY"]
   ```
9. Load `.env` and start Codex from that shell (`set -a; . ./.env; set +a` then `codex`).

Undo: `codex mcp remove postiz` + delete the `.env` block. Tearing down the deploy is separate (`docker compose down -v` for Path A).
...
## Validation
Ask Codex: "Using the Postiz MCP, list my connected social channels." Expect JSON with ≥1 channel ID.
...
- `POSTIZ_URL` pointing at `localhost` only works while the local deploy is up — if the MCP starts before docker does, tools fail with a connection error, not an auth error.
