...
If Postiz isn't an option, there's a ⚠️ direct-API Bash fallback (Codex CLI only).
...
No `codex mcp add` for this recipe — Postiz's MCP is already registered and this rides on it.
...
## Direct-API fallback (⚠️ Codex CLI only — only if Postiz isn't used)
...
A drafting skill produces the text; a Bash call publishes it. This path needs a shell, so it works in the Codex CLI and does not travel to MCP-only clients — which is exactly why Postiz is the primary path.
