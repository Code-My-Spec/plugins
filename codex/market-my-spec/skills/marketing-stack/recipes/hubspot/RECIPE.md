...
3. **Register the MCP:** pick the active community HubSpot MCP — check https://github.com/search?q=mcp+hubspot&type=repositories for current maintenance. At time of writing, suggested:
   ```bash
   codex mcp add hubspot -- npx -y <hubspot-mcp-package>
   ```
   (User may need to verify the specific package; this ecosystem moves.)
4. Add env vars to `.env`, then name them in `~/.codex/config.toml` (names only — the token never goes in this file):
   ```toml
   [mcp_servers.hubspot]
   command = "npx"
   args = ["-y", "<hubspot-mcp-package>"]
   env_vars = ["HUBSPOT_PRIVATE_APP_TOKEN", "HUBSPOT_PORTAL_ID"]
   ```
5. Load `.env` and start Codex from that shell (`set -a; . ./.env; set +a` then `codex`).

Undo: `codex mcp remove hubspot` + delete the `.env` block. Delete the Private App in HubSpot to revoke the token.
