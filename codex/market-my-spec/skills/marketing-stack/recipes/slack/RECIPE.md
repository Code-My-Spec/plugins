...
2. **Register the MCP:**
   ```bash
   codex mcp add slack -- npx -y slack-mcp-server@latest
   ```
   (Verify the exact npm package name in the upstream README; the canonical run path is the Go binary — `go install github.com/korotovsky/slack-mcp-server/cmd/slack-mcp-server@latest` then point `command` at that binary instead of npx.)
3. Add env vars to `.env`, then name them in `~/.codex/config.toml` (names only — tokens never land in this file):
   ```toml
   [mcp_servers.slack]
   command = "npx"
   args = ["-y", "slack-mcp-server@latest"]
   env_vars = ["SLACK_MCP_XOXC_TOKEN", "SLACK_MCP_XOXD_TOKEN", "SLACK_MCP_ADD_MESSAGE_TOOL", "SLACK_MCP_MARK_TOOL"]
   ```
   (Name only the token vars for the path you chose. Listing a var that isn't set is harmless — it just isn't forwarded.)
...
5. Same wiring as Path A, naming `SLACK_MCP_XOXB_TOKEN` / `SLACK_MCP_XOXP_TOKEN` in `env_vars` instead.

Undo: `codex mcp remove slack` + delete the `.env` block.
...
## Validation
Ask Codex: "Using the slack MCP, list channels in my workspace." Expect JSON array with `id` and `name` fields.
...
- **The old `@modelcontextprotocol/server-slack` reference server was archived May 2025.** Don't wire to that package. Zencoder forked it, but korotovsky's server has broader tool surface (15 vs 8 tools) and active maintenance.
...
- An empty `channels_list` has two very different causes — admin restrictions (above) or tokens that never reached the server. Rule out the second first: check `env_vars` and that `.env` was loaded before Codex started.
