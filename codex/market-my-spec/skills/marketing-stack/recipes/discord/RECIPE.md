...
3. **Register the MCP:**
   ```bash
   codex mcp add discord -- npx -y mcp-discord
   ```
4. Add env vars to `.env`, then name them in `~/.codex/config.toml` (names only — the bot token never lands in this file):
   ```toml
   [mcp_servers.discord]
   command = "npx"
   args = ["-y", "mcp-discord"]
   env_vars = ["DISCORD_TOKEN", "DISCORD_GUILD_ID"]
   ```
5. Load `.env` and start Codex from that shell (`set -a; . ./.env; set +a` then `codex`).
6. After the MCP first runs, verify the bot shows **online** in your server's member list. It stays offline until the MCP process starts and calls `discord_login`.

Undo: `codex mcp remove discord` + delete the `.env` block. Delete the application in the Developer Portal to fully retire the bot.
...
## Validation
Ask Codex: "Using the discord MCP, list the servers the bot is connected to." Expect JSON array with at least one entry containing `id` and `name`.
...
- Bot never comes online + no error → the token didn't reach the server. Check `env_vars` and whether `.env` was loaded before Codex started.
