...
**No MCP wrapper.** ⚠️ Codex CLI only (uses Bash + curl) — it won't travel to MCP-only clients. User decision: we're not building or adopting a Resend MCP in v1.
...
No MCP registration — this recipe is Bash + curl against the REST API, so `.env` is the whole install. Load it into your shell before running the calls (`set -a; . ./.env; set +a`).

Undo: delete the `.env` block and revoke the key in the Resend dashboard.
...
- **No MCP in v1.** Works in the Codex CLI via Bash; doesn't work in MCP-only clients that can't run shell commands.
