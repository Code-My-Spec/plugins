...
5. **Register the MCP (choose one):**
   ...
   codex mcp add meta-mcp -- npx -y meta-mcp

   # Option B — pipeboard-co/meta-ads-mcp (Meta Business Partner badged, streamable HTTP)
   # Registers with --url instead of a command; see the repo for the endpoint.
   #   codex mcp add meta-ads-mcp --url <endpoint-from-repo>
   ```
6. Add env vars to `.env`, then name them in `~/.codex/config.toml` (names only — the app secret and token never land in this file):
   ```toml
   [mcp_servers.meta-mcp]
   command = "npx"
   args = ["-y", "meta-mcp"]
   env_vars = ["META_APP_ID", "META_APP_SECRET", "META_AD_ACCOUNT_ID", "META_ACCESS_TOKEN"]
   ```
7. Load `.env` and start Codex from that shell (`set -a; . ./.env; set +a` then `codex`).

Undo: `codex mcp remove meta-mcp` + delete the `.env` block. Revoke the token in Business Settings if retiring it.
...
- 60-day token expiry is the recurring failure. When tools start 401ing, check expiry before anything else — or use a System User token and stop thinking about it.
