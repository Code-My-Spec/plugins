...
# - No secrets in recipe templates, and no non-Codex install instructions
#
...
if [ -n "${SKILL_DIR:-}" ]; then
  ...
  SKILL="."
elif [ -d "./codex/market-my-spec/skills/marketing-stack" ]; then
  SKILL="./codex/market-my-spec/skills/marketing-stack"
...
for f in SKILL.md steps/inventory.md steps/install_recipe.md steps/install_for_channel.md steps/fix_recipe.md steps/blueprint.md scripts/mcp_env_wrapper.sh; do
  if [ -f "$f" ]; then
    ok "$f"
  else
    bad "$f missing"
...
# ---------------------------------------------------------------------------
# Check 7: Codex-native install instructions only.
# Every command we tell the user to run must be a real Codex command, and the
# .env-only rule forbids ${VAR} in MCP config (Codex never expands it).
# ---------------------------------------------------------------------------
echo "## Codex-native instructions"

# Patterns are bracket-escaped so this script does not match itself when it
# scans the tree it lives in: a bracketed character matches as a regex but the
# forbidden word is never spelled literally in this file.
LEGACY_HARNESS='cl[a]ude mcp|\.mc[p]\.json|sett[i]ngs\.json|~/\.cl[a]ude'
if grep -rniE "$LEGACY_HARNESS" . >/dev/null 2>&1; then
  bad "found install instructions for a different harness — this skill must be Codex-native"
  grep -rniE "$LEGACY_HARNESS" . | head -5 | sed 's/^/      /'
else
  ok "no non-Codex MCP install instructions"
fi

# Codex does not expand ${VAR} in config.toml, so an env map using it is a live
# bug: the server receives the literal string. env_vars is the supported path.
INTERPOLATED_ENV='env = [{].*[$][{]'
if grep -rnE "$INTERPOLATED_ENV" . >/dev/null 2>&1; then
  bad "found an MCP env map using variable interpolation — Codex does not expand it; use env_vars"
  grep -rnE "$INTERPOLATED_ENV" . | head -5 | sed 's/^/      /'
else
  ok "no variable interpolation in MCP env maps"
fi
echo
