...
└── steps/
    ...
    ├── 03_persona_research.md   ← runs the customer research
    ├── 04_beachhead.md
...
3. If the user gave you a URL or product name in arguments, fetch and read it with web search before asking a single question. Don't make the user type things you can read.
...
| 3 | Persona research | **External research** | `marketing/03_personas.md` + `marketing/research/` |
...
2. Follow the instructions in that doc (interview questions, research briefs, or synthesis guidance).
...
- **Ground personas in evidence.** Step 3 runs real external research because invented personas are worse than no personas. Don't skip it.
...
3. Identify which step(s) need updating — often one or two, not all nine. Common patterns:
   ...
   - New visual identity work shipped externally (design tool / hired designer) → update step 0's visual identity section, then check step 5 component 7 still aligns
...
- Set up analytics, CRMs, email tools, or ad accounts — that's tooling setup (`/marketing-stack`)
...
The goal is a compact, evidence-backed strategy the user can execute against starting tomorrow morning, and iterate on over time. Once the strategy exists, `/marketing-stack` wires up the infrastructure the chosen channels need, and `/daily-plan` turns the 90-day plan into today's work.
